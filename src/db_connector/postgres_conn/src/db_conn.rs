...
impl PostgresConnection {
    ...
    pub(crate) fn new(info : CommonSqlConnectionInfo) -> Result<Self, Box<dyn Error>> {
        ...
        let conn = match postgres::Client::connect(url.as_str(), postgres::NoTls) {
            Ok(ok) => Ok(ok),
            Err(err) => common_make_err!(connection, GetConnectionFailedError, "{}", err.to_string())
        }?;
    ...
    fn execute(&mut self, query : &'_ str, param : &'_ [CommonValue]) -> Result<common_conn::CommonSqlExecuteResultSet, Box<dyn std::error::Error>> {
        let pg_param  = param.iter().map(| x | {
            let convert: Result<&(dyn ToSql + Sync), Box<dyn Error>> = match x {
                ...
                _ => {
                    let err_error : Result<&(dyn ToSql + Sync), Box<dyn Error>> = common_make_err!(system, CriticalError, "not support type({:?}), return null", x);
                    err_error
                }
            };
            convert
        ...
        let rows = match self.client.query(query, pg_param.as_slice()) {
            Ok(ok) => Ok(ok),
            Err(err) =>  common_make_err!(connection, CommandRunError, "{}",err.to_string())
        }?;
        ...
        for row in &rows {
            ...
            for col_idx in 0..cols_t.len() {
                let d = match cols_t[col_idx] {
                    ...
                    _ => {
                        common_make_err!(data, ResponseScanError, "not support this type({}), return NULL", cols_t[col_idx])
                    }
                }?;
