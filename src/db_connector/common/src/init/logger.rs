    ...
    LOGGER_INIT_ONCE.call_once(|| {
        ...
        if log_file.is_some() {
            ...
            if output_opt.is_none() {
                ret = make_err_crate!(system, ApiCallError, "sh failed");
                return;
            }
...

            {
                ...
                if chk_write.is_err() {
                    ret = make_err_crate!(system, ApiCallError, "{}", chk_write.unwrap_err());
                    return;
                }
            }
        ...
        unsafe {
            ...
            ret = match ftail.init() {
                Ok(_) => Ok(()),
                Err(e) => make_err_crate!(system, ApiCallError, "{}", e)
            }
        }
    });
