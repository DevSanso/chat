...
impl CommonSqlConnection for DuckDBConnection {
    fn execute(&mut self, query : &'_ str, param : &'_ [CommonValue]) -> Result<common_conn::CommonSqlExecuteResultSet, Box<dyn std::error::Error>> {
        let mut prepare = self.client.prepare(query).map_err(|x| {
            let e : Result<(), Box<dyn Error>> = common_make_err!(connection, ConnectionApiCallError, "{}", x);
            e.unwrap_err()
        })?;
        ...
        let duck_param  = param.iter().map(| x | {
            let convert: Result<&(dyn ToSql), Box<dyn Error>> = match x {
                ...
                _ => {
                    let err_error : Result<&(dyn ToSql), Box<dyn Error>> = common_make_err!(system, CriticalError, "not support type({:?}), return null", x);
                    err_error
                }
            };
            convert
        ...
        let mut rows = prepare.query(duck_param.as_slice()).map_err(|x| {
            let err_error : Result<(), Box<dyn Error>> = common_make_err!(connection, CommandRunError, "{}", x);
        ...
        loop  {
            let row = rows.next();
            if row.is_err() {
                return common_make_err!(connection, ConnectionApiCallError, "{}", row.err().unwrap());
            }
            ...
            for idx in 0..col_count {
                let data = match schema.field(idx).data_type() {
                    ...
                    _ => common_make_err!(system, NoSupportError, "not exists col type data")
                };
    ...
    fn get_current_time(&mut self) -> Result<std::time::Duration, Box<dyn Error>> {
        let data : i64 = self.client.query_row(
            ...
            .map_err(|x| {
                let e : Result<(), Box<dyn Error>> = common_make_err!(data, ResponseScanError, "{}", x);
                e.unwrap_err()
            })?;
